<template>
    ...
    <ul class="list">
        <li 
          ...
          :id="index" 
          class="item">
          <input type="text" v-model="item.todo" class="input">
          <span class="name">{{ item.todo }}</span>

          <div class="buttons">
            <button @click="onEditingMode" class="btn btn--edit">&#9998;</button>
            <button v-if="!item.completed" @click="markComplete(item.id)" class="btn btn--complete">&#10003;</button>
            <button v-else @click="removeFromList(item.id)" class="btn btn--remove">X</button>
          </div>
        </li>
    </ul>
</template>
